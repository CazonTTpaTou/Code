	...
	while(!(inLine.equalsIgnoreCase("F"))) {
		...
		if (inLine.equalsIgnoreCase("F")){System.out.println("Sortie de l'application");}
				...
				if(sc2.hasNextInt()) {Montant = sc2.nextInt();}
					...
					switch(typeo) {
					case s3:
						try {Lyon.rechercherCompte(N_Compte).debiter(Montant);
						System.out.println(Montant+ " ont ete debite du compte n° " + N_Compte);}

						catch(DebitNon_Autorise_Exception e) {
							e.printStackTrace();
							System.out.println("Attention Votre solde est négatif");
							System.out.println("Votre opération de débit sur le compte Epargne a été annulée...");}
