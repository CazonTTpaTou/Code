...
if(($_POST['Client']!='inconnu')&&($_POST['Client']!='')) {$clause.="and l.isbn like '%".$_POST['Client']."%' ";}
if(($_POST['Commande']!='inconnu') && ($_POST['Commande']!='')) {
								 $clause.="  and p.d_end>=curdate() and p.prix".$op.$_POST['Commande'];}
if(($_POST['Date_d']!='inconnu')&&($_POST['Date_d']!=''))  {$clause.="and l.titre like'%".$_POST['Date_d']."%' ";}
if(($_POST['Date_f']!='inconnu')&&($_POST['Date_f']!='')) {$clause.="and a.nom like '%".$_POST['Date_f']."%' ";}
if($_POST['Ordre']!='inconnu') {$ordre=" order by ".$_POST['Ordre']." ";}
	else{$ordre='';}
...
$sq="select l.numero,l.isbn,group_concat(auteurs.auteur) as Auteurs,l.titre,p.prix from livres as l inner join prix as p on l.numero=p.id_livre inner join liv_aut as a on a.id_livre=l.numero inner join auteurs on auteurs.id_auteur=a.id_auteur where l.numero>0 ".$clause." group by a.id_livre".$ordre.";";
...
if ($ligne==0) {foreach($row as $key => $value) {$Code[]='<td id="'.$key.'" class="entete" onclick="livre(this.id)">'.$key.'</td>';}
