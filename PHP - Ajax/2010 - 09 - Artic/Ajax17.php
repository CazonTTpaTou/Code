...
if(($_POST['Date_d']!='inconnu')&&($_POST['Date_d']!=''))  {$clause.="and comi.date>='".$_POST['Date_d']."' ";}
if(($_POST['Date_f']!='inconnu')&&($_POST['Date_f']!='')) {$clause.="and comi.date<='".$_POST['Date_f']."' ";}

if(($_POST['Nom']!='inconnu')&&($_POST['Nom']!='')) {$clause.="and nom  like '%".$_POST['Nom']."%' ";}
if(($_POST['CP']!='inconnu')&&($_POST['CP']!='')) {$clause.="and code_postal like '".$_POST['CP']."%' ";}
if(($_POST['Code_C']!='inconnu')&&($_POST['Code_C']!='')) {$clause.="and id_client =".$_POST['Code_C']." ";}

if($_POST['Ordre']!='inconnu') {$clause.=" order by ".$_POST['Ordre']." ";}

$nb=0;
if($_POST['Nom']=='inconnu') {$nb++;}
if($_POST['CP']=='inconnu') {$nb++;}
if($_POST['Code_C']=='inconnu') {$nb++;}
...
$sq="select coalesce(ex.date,0) as date_expedition,id_commande,comi.date,nom,code_postal,ville,montant,nb_livre from comi left join expedition as ex on ex.numero_co=comi.id_commande where id_commande>0 ".$clause." ;";
...
if ($ligne==0) {foreach($row as $key => $value) {$Code[]='<td id="'.$key.'" class="entete" onclick="expedition_r(this.id)">'.$key.'</td>';}
...
foreach($row as $key => $value) {if($colonne==0) {$identifiant=$value;}
			         if($key=='date_expedition') {if($value==0) {$value='Non Expédié';
									     $couleur=' class="fond3" ';}}
