...
if (isset($_GET['Commande']) && ($_GET['Commande']!=0)) {
	$req="UPDATE commande set Numero_Client=".$_POST['N_Client'].",date='".$_POST['date']."',port='".$_POST['port']."',catalogue='".$_POST['Catalogue']."',livraison='".$_POST['Livraison']."' where numero_com=".$_GET['Commande'].";";
	...
	if($success) {
	      		$_SESSION['Message']='Commande N° '.$_GET['Commande'].' modifiée !!!';
	                header('Location:Intro.php?Operation=5&Commande='.$_GET['Commande']);}
	...
	if($success) {$_SESSION['Last_Insert']=Last_Insertion_Com();
	   	      ...
	   	      header('Location:Intro.php?Operation=2&id='.$_SESSION['Last_Insert']);}
