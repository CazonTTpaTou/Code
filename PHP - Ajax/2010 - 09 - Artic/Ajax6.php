...
if(($_POST['Client']!='inconnu')&&($_POST['Client']!='')) {$clause.="and co.numero_client =".$_POST['Client']." ";}
if(($_POST['Commande']!='inconnu') && ($_POST['Commande']!='')) {
								 $clause.="and co.port".$op.$_POST['Commande']." ";}
if(($_POST['Date_d']!='inconnu')&&($_POST['Date_d']!=''))  {$clause.="and co.date>='".$_POST['Date_d']."' ";}
if(($_POST['Date_f']!='inconnu')&&($_POST['Date_f']!='')) {$clause.="and co.date<='".$_POST['Date_f']."' ";}
...
$sq="select co.numero_com,co.date,cl.nom,cl.prenom,cl.code_postal,cl.ville,co.port as montant,(case co.catalogue when 2 then 'Un an' when 1 then '6 mois' end) as abonnement from commande as co inner join clients as cl on cl.numero=co.numero_client where co.numero_com>=3141 ".$clause." ;";
...
if ($ligne==0) {foreach($row as $key => $value) {$Code[]='<td id="'.$key.'" class="entete" onclick="commande(this.id)">'.$key.'</td>';}
