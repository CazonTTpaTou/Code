...
function Connexion() {
	...
	mysql_select_db("difedit_com_droite_ligne",$connectID);
	return $connectID;}
...
//------------ Fonction qui de déterminer le numéro d'incrémentation de la dernière livraison insérée

function Last_Insertion_Liv() {

	$connectID=Connexion();
	$sql="select numero_com as last from livraison order by numero_com desc limit 1;";
	//$sql="select last_insert_id() as last;";
	$result = mysql_query($sql,$connectID);
...
//------------ Fonction qui de déterminer le numéro d'incrémentation du dernier catalogue inséré

function Last_Insertion_Cat() {

	$connectID=Connexion();
	$sql="select num_cat as last from catalogue order by num_cat desc limit 1;";
	//$sql="select last_insert_id() as last;";
	$result = mysql_query($sql,$connectID);
...
function cocher_R($req,$nom,$table,$champ1,$champ2,$valeur) {
...
//-------------- Fonction qui construit un menu par bouton unique à cocher

function cocher_RR($req,$nom,$table,$champ1,$champ2,$valeur) {
...
//$req="select * from ".$table." order by ".$champ2." ;";
	...
	$result = mysql_query($req,$connectID);
		...
		while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
			...
			if($valeur==$row[$champ1]) {$selec='Checked';}
				
			print '<input type="radio" name="'.$nom.'" value="'.$row[$champ1].'" '.$selec.' >'.$row[$champ2];
			//enregistre('abc','<input type="Checkbox" name="'.$nom.'" value="'.$row[$champ1].'" '.$selec.' >'.$row[$champ2]);
			print '<br/>';
			//print '<br/>';
			}
