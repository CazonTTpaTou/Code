...
if(($_POST['Client']!='inconnu')&&($_POST['Client']!='')) {$clause.="and nom like'".$_POST['Client']."%' ";}
if(($_POST['Commande']!='inconnu') && ($_POST['Commande']!='')) {
								 $clause.="and prenom like'".$_POST['Commande']."%' ";}
if(($_POST['Date_d']!='inconnu')&&($_POST['Date_d']!=''))  {$clause.="and naissance>='".$_POST['Date_d']."' ";}
if(($_POST['Date_f']!='inconnu')&&($_POST['Date_f']!='')) {$clause.="and mort<='".$_POST['Date_f']."' ";}
...
$sq="select id_auteur,nom,prenom,naissance,mort from auteurs where id_auteur>0 ".$clause." ;";
...
if ($ligne==0) {foreach($row as $key => $value) {$Code[]='<td id="'.$key.'" class="entete" onclick="auteur(this.id)">'.$key.'</td>';}
...
if ($ligne==0) {$Code[]='<br/>Aucune donnée ne correspond à votre recherche...';}
	else {
		if($ligne>1) {$s='s';}
		$Code[0]='<br/><p>Résultat: '.$ligne.' proposition'.$s.' trouvée'.$s.' (cliquer sur l\'en tête de colonne pour trier): </p>';
	}
