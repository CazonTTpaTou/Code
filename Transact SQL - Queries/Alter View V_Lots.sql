...
/****** Object:  View [dbo].[V_LOTS]    Script Date: 13/02/2017 11:13:15 ******/
SET ANSI_NULLS ON
GO
...
SELECT TOP (100) PERCENT
	...
	CASE WHEN T_LOT_1.IdAtelierFinal = 1 THEN 'EDF ENR PWT' ELSE 'PV Alliance' END AS Societe
    , T_LOT_1.Lot
    , CASE WHEN T_LOT_1.Groupable = - 1 THEN 'oui' ELSE 'non' END AS Groupable
    , CASE WHEN dbo.V_LOT_GROUPE_SERIG_COUNT.GroupLot IS NOT NULL THEN 'oui' ELSE 'non' END AS Groupe
    , dbo.V_LOT_GROUPE_SERIG_COUNT.NbLots AS [Nb Lots]
 	-- V2.9.8 - Début AJOUT 	
    , CASE WHEN dbo.V_LOT_GROUPE_TRI_COUNT.GroupLot IS NOT NULL THEN 'oui' ELSE 'non' END AS [Groupe TRI]
    , dbo.V_LOT_GROUPE_TRI_COUNT.NbLots AS [Nb Lots TRI]
 	-- V2.9.8 - Début AJOUT 	
    , CASE WHEN T_LOT.Lot IS NULL THEN T_LOT_1.Lot ELSE T_LOT.Lot END AS [Lot Serig]
    , T_Statut_7.Statut
	...
	, Lin.[Lingot 1]
	, Lin.[Lingot 2+]
	, ISNULL('Mep: ' + T_LOT_1.ObservMeP,'') + ISNULL('Textu: ' + T_LOT_1.ObservTextu, '') + ISNULL('Diff: ' + T_LOT_1.ObservDiffu, '') 
					                         + ISNULL('Desox: ' + T_LOT_1.ObservDesox, '') + ISNULL('Pecvd: ' + T_LOT_1.ObservPecvd, '') 
											 + ISNULL('Seri: ' + T_LOT_1.ObservSerig, '') + ISNULL('Tri: ' + T_LOT_1.ObservTri, '') AS Observations
	, 1 AS Compte
	, T_LOT_1.OF1
	, T_LOT_1.OF2
	, CASE WHEN T_LOT_1.ManuMeg IS NULL THEN '' WHEN T_LOT_1.ManuMeg = 1 THEN 'saisie' ELSE 'log machine' END AS [Meg Mode]
	...
	, CASE WHEN T_LOT_1.ManuMeP = 1 THEN 'saisie' ELSE 'log machine' END AS [Mep Mode]
	...
	, CASE WHEN T_LOT_1.ManuTextu = 1 THEN 'saisie' ELSE 'log machine' END AS [Textu Mode]
	...
	, CASE WHEN T_LOT_1.ManuDiffu = 1 THEN 'saisie' ELSE 'log machine' END AS [Diff Mode]
	...
	, T_LOT_1.QteEntrDiffu - T_LOT_1.QteSortDiffu - ISNULL(T_LOT_1.QteVenteDiffu, 0) AS [Diff Rej Total]
	...
	, CASE WHEN T_LOT_1.ManuDesox <> 1 THEN 'saisie' ELSE 'log machine' END AS [Desox Mode]
	...
	, CASE WHEN T_LOT_1.ManuPecvd = 1 THEN 'saisie' ELSE 'log machine' END AS [Pecvd Mode]
	...
	, CASE WHEN T_LOT_1.ManuSerig = 1 THEN 'saisie' ELSE 'log machine' END AS [Seri Mode]
	...
	, T_LOT_1.QteEntrSerig - T_LOT_1.QteSortSerig - ISNULL(T_LOT_1.QteVenteSerig, 0) AS [Seri Rej Total], T_LOT_1.QteManqSerig AS [Serig Qte Manq]
	...
	, CASE WHEN T_LOT_1.ManuTri = 1 THEN 'saisie' ELSE 'log machine' END AS [Tri Mode]
	...
	, T_LOT_1.QteEntrTri - T_LOT_1.QteSortTri AS [Tri Rej Total]
    ...
    ,CASE WHEN T_LOT_1.IdAtelierInitial = 1 THEN 'EDF ENR PWT' ELSE 'PV Alliance' END AS Atelier_Initial
    ,CASE WHEN T_LOT_1.IdAtelierFinal = 1 THEN 'EDF ENR PWT' ELSE 'PV Alliance' END AS Atelier_Final
		
	-------------------------------------------------------------------------------------
	-- Nouveaux champs V_Lots In Cell Line pour reporting SAS - 04/04/2014 --------------
	-------------------------------------------------------------------------------------	
	
	,T_Lot.OnOffLinePecvd AS [PECVD_OnOffLine]
	...
	,T_Lot.OnOffLineSerig AS [SERIG_OnOffLine]
	...
	,T_Lot.OnOffLineTri AS [TRI_OnOffLine]
	...
	-- V2.9.7b - Début AJOUT
	,ISNULL(T_LOT_1.QteManqEntreePecvd,0) 	AS [PECVD_QuantiteManqEntree]
	,ISNULL(T_LOT_1.QteAjoutPecvd,0) 	AS [PECVD_QuantiteAjout]
	,ISNULL(T_LOT_1.RejOperEbrechPecvd, 0) + ISNULL(T_LOT_1.RejOperAspectPecvd, 0) + 
	 ISNULL(T_LOT_1.RejOperCassePecvd, 0) + ISNULL(T_LOT_1.RejParamPecvd, 0)  AS [PECVD_QuantiteRejetOper]
	,ISNULL(T_LOT_1.QteManqEntreeSerig,0) 	AS [SERIG_QuantiteManqEntree]
	,ISNULL(T_LOT_1.QteAjoutSerig,0) 	AS [SERIG_QuantiteAjout]
	,ISNULL(T_LOT_1.RejOperEbrechSerig, 0) + ISNULL(T_LOT_1.RejOperAspectSerig, 0) + 
	 ISNULL(T_LOT_1.RejOperCasseSerig, 0) + ISNULL(T_LOT_1.RejOperGradeBSerig, 0) +
	 ISNULL(T_LOT_1.RejParamSerig, 0)  AS [SERIG_QuantiteRejetOper]
	,ISNULL(T_LOT_1.QteManqEntreeTri,0) 	AS [TRI_QuantiteManqEntree]
	,ISNULL(T_LOT_1.QteAjoutTri,0) 	AS [TRI_QuantiteAjout]
	,ISNULL(T_LOT_1.RejOperEbrechTri, 0) + ISNULL(T_LOT_1.RejOperAspectTri, 0) + 
	 ISNULL(T_LOT_1.RejOperCasseTri, 0) + ISNULL(T_LOT_1.RejOperGradeBTri, 0) +
	 ISNULL(T_LOT_1.RejParamTri, 0)  AS [TRI_QuantiteRejetOper]
	,ISNULL(T_LOT_1.RejEquipAspectInit,0) 	AS [SERIG_RejEquipAspectInitial]
	-- V2.9.7b - Début AJOUT
	...
	,Convert(Varchar(15),
				'Semaine n° ' 
					...
					+ Convert(Varchar(2),DatePart(WEEK,T_LOT_1.DateDebPosteSortTri)-1)) AS [Tri Date Poste Semaine]
...
FROM         
		...
		-- V2.9.8 - Début MODIFICATION 	
		-- Nombre de sous lots pour lot groupé en SERIGRAPHIE
		LEFT OUTER JOIN dbo.V_LOT_GROUPE_SERIG_COUNT 
			ON T_LOT_1.IdLot = dbo.V_LOT_GROUPE_SERIG_COUNT.GroupLot 
		-- Nombre de sous lots pour lot groupé en TRI
		LEFT OUTER JOIN dbo.V_LOT_GROUPE_TRI_COUNT 
			ON T_LOT_1.IdLot = dbo.V_LOT_GROUPE_TRI_COUNT.GroupLot 
		-- V2.9.8 - Début MODIFICATION 			
		...
		LEFT OUTER JOIN dbo.T_Forme ON T_LOT_1.IdForme = dbo.T_Forme.Id 
