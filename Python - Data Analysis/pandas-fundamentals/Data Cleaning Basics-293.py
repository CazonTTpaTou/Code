...
def cleaner(label):
    label = label.replace('Operating System','os')
    
    return label.strip().replace(' ','_').replace('(','').replace(')','').lower()
...
laptops['ram'] = laptops['ram'].str.replace('gb','').str.replace('GB','')
...
#laptops_dtypes == laptops_cleaned_dtypes
