...
class ValidationCroisee:
    ...
    def entrainement_Modele_KFold(self,numeroFold,hyperParametres):
        ...
        x = self.returnFeatures(self.returnSetTraining())
        y = self.returnClassification(self.returnSetTraining())
        print(x.shape)
        print(y.shape)
        print(type(y))
        ...
        xx = self.returnFeatures(self.returnSetTest())
        yy = self.returnClassification(self.returnSetTest())
        print(xx.shape)
        print(yy.shape)
        #print(classification.score(
        #                            self.returnFeatures(self.returnSetTest()),
        #                            self.returnClassification(self.returnSetTest())))
        classification.score(xx, yy)
...
validation.entrainement_Modele_KFold(1,1)
