...
def assign_to_cluster(row):
    distances = dict()
    for index,value in centroids_dict.items():
        """
        print(value)
        print([row['ppg'], row['atr']])
        """
        distances[index] = calculate_distance(
                                                value,
                                                [row['ppg'], row['atr']])

    return min(distances.keys(),key=(lambda x:distances[x]))
...
def recalculate_centroids(df):
    ...
    for cluster_id in range(0, num_clusters):
        row_cluster = df[df['cluster']==cluster_id]
        coordinates = [row_cluster['ppg'].mean(), row_cluster['atr'].mean()]
