...
base_10_100 = int(str(int("100",2)),10)
print(base_10_100)
...
value = "ea"
hex_ea = hexadecimal_add(value,"2")

value = "f"
hex_ef = hexadecimal_add(value,"e")
...
head = sentences_cia[0]
data = sentences_cia[1:]
sentences_cia = pd.DataFrame(data,columns=head)

print(sentences_cia.iloc[0,:])
...
def clean_statement(row):
    clean_row = [x for x in row if ord(x) in good_characters]
...
filtered_tokens = [x for x in statement_tokens if len(x)>=5]
...
filtered_token_counts = filtered
...
def common_year(year,most):
    df = sentences_cia[sentences_cia['year']==str(year)]
    
    all_sentences = "".join(df['statement'])
    all_words = all_sentences.split(' ')
    all_true_words = [w for w in all_words if len(w)>=5]
                        
    count_words =  Counter(all_true_words)

    return count_words.most_common(most)
                            
common_2000 = common_year(2000,2)
common_2002 = common_year(2002,2)
common_2013 = common_year(2013,2)
