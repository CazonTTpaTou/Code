...
ensemble=list()
ensemble.append(clf)
ensemble.append(clf2)

for ens in ensemble:
    print(roc_auc_score(test["high_income"],
                        ens.predict(test[columns])))
...
rounded = np.round(combined,0)

auc = roc_auc_score(test['high_income'],rounded)
...
mean_predict = np.round(sum(predictions)/len(predictions),0)

auc = roc_auc_score(test['high_income'],mean_predict)
...
auc=roc_auc_score(test['high_income'],
...
auc = roc_auc_score(train['high_income'],
                    predictions)
