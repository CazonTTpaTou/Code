...
/****** Object:  View [dbo].[V_IN_LINE_CELL_24_HEURES]    Script Date: 12/03/2014 13:08:44 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[V_IN_LINE_CELL_24_HEURES]
AS
...
Entree_Wafer_Ligne AS
		...
		(SELECT 
					...
					WHERE 	
						datediff(dd,E01.[DateTime],getdate()) <= 1) 
------------------------------------------------------------------------						
------------------------------------------------------------------------				
...
SELECT
		...
		FROM
			...
			Entree_Wafer_Ligne AS E01
			
				LEFT OUTER JOIN dbo.WAFER_SNL AS E02
