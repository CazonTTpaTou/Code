...
GO
IF OBJECT_ID('dbo.T_NIVEAU_NIV') IS NOT NULL
   EXEC ('DROP TABLE dbo.T_NIVEAU_NIV;');
GO
...
CREATE TABLE dbo.T_NIVEAU_NIV
(NIV_ID              SMALLINT IDENTITY PRIMARY KEY,
 NIV_LIBELLE         VARCHAR(32));
GO
...
(
	...
	NIV_ID                    smallint              NULL FOREIGN KEY REFERENCES dbo.T_NIVEAU_NIV (NIV_ID),
...
SET IDENTITY_INSERT dbo.T_NIVEAU_NIV ON;
INSERT INTO dbo.T_NIVEAU_NIV (NIV_ID, NIV_LIBELLE)
VALUES
...
(4, 'Équipement'),
(5, 'Sous-équipement');
SET IDENTITY_INSERT dbo.T_NIVEAU_NIV OFF;
GO
...
INSERT INTO dbo.T_SYSTEME_PRODUCTION_STP 
      (STP_ID, STP_ID_PERE, NIV_ID, STP_LIBELLE, STP_ACTIF, STP_ORDRE, 
...
UPDATE sp
SET    NIV_ID = COALESCE(pere.NIV_ID + 1, 1)
...
AS
SELECT STP_ID, NIV_LIBELLE, STP_ID_PERE, sp.NIV_ID, STP_LIBELLE, STP_ACTIF, STP_ORDRE, 
       STP_POURCENT_DEPENDANCE, STP_CODE_EQUIPEMENT, STP_NUMERO_FOURNISSEUR, STP_NUMERO_SERIE, 
	   Sysutc, Sysdhc, Sysutm, Sysdhm
FROM   dbo.T_SYSTEME_PRODUCTION_STP AS sp
       INNER JOIN dbo.T_NIVEAU_NIV AS n
	         ON sp.NIV_ID = n.NIV_ID;
GO
...
SELECT STP_ID AS idUAP, 
       ...
       STP_POURCENT_DEPENDANCE AS PourcentageDependance, NIV_ID
...
SELECT STP_ID AS idEquipement, STP_LIBELLE AS Libelle, STP_ID_PERE AS idProcede, 
       ...
       STP_NUMERO_SERIE AS NumeroSerie, NIV_ID
FROM   dbo.T_SYSTEME_PRODUCTION_STP
WHERE  NIV_ID = 3
WITH CHECK OPTION;
GO
