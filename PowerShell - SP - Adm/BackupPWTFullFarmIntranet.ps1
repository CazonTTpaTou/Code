...
# version 1.1 mise à jour le 3/05/2016
...
$c1 = $Error.Count
Backup-SPFarm -BackupMethod Full -Directory $spbackupPath
$c2 = $Error.Count

if ($c2 -eq $c1)
{
$sub = "Succes de la sauvegarde de la ferme SharePoint de production"
...
{
$sub = "Echec de la sauvegarde de la ferme SharePoint de production"
}
...
# Commande ajoutée pour simplifier le transfert de l'intranet de la production vers la recette, sans pour autant toucher à la maquette 
# Nintex Forms stockée dans la base de contenu WSS_Content_Intranet de la recette.

Backup-SPSite http://intranet.photowatt.local -Path "\\cifs-fas2240\Backups\SQLPhotowatt\SharePointPP\IntranetProd.bak" -Force
