...
sAMAccountName=upcase(sAMAccountName);
cn=upcase(cn);
run;
...
/******************************************************************************************/
proc sql noprint;
   select count(*) into :ldapusers_nobs from &extractlibref..ldapfusers;
   quit;
   
data _null_;
   if &ldapusers_nobs = 0 then do;
      put "ERROR: User extraction failed.  The dataset &extractlibref..ldapfusers contains no observations.";
...
/* phone, email, and login from the &extractlibref..ldapfusers extracted above.            */
/******************************************************************************************/
...
data &persontbla                      /* Macros to define Normalized Tables from %mduimpc */
     ...
     /*  DisplayName is in the input ldapfusers dataset already */
     /* title is in the input ldapfusers dataset already */
     name=sAMAccountName;
     /* since display name is now a field, in the Person object, leave the description blank. */
     description="PHOTOWATT"!!" / "!!trim(left(department))!!" / "!!sAMAccountName;  
     output &persontbl;
...
data &extractlibref..ldapgrps
     ...
     filter="(&(name<=A)(objectClass=group))";
     %ldapextrgroups  
...
proc sql;
delete from &extractlibref..ldapgrps;
quit;

proc sql;
insert into &extractlibref..ldapgrps (distinguishedName,member,name,description)
select department,sAMAccountName,department,"PHOTOWATT / "!!trim(left(department))
from &extractlibref..ldapfusers;
quit;
...
/* membership from the &extractlibref..ldapfusers extracted above.             */
/******************************************************************************/
...
%macro transmemkeyid;
   %if %upcase(&keyidvar)^=DISTINGUISHEDNAME %then %do;
      proc sql;
         update &idgrpmemstbl
            set memkeyid = 
                case when (select unique &keyidvar from &extractlibref..ldapfusers 
                              where memkeyid = distinguishedName)            
                                 is missing then memkeyid           
                     else (select unique &keyidvar from &extractlibref..ldapfusers            
                              where memkeyid = distinguishedName)           
                end;           
      quit;     
   %end;
