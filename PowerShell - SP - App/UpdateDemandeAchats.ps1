...
$timestamp = "MAJ_DA_" + $timestamp
$LogFile = "C:\Logs\DA\$timestamp.log"
...
function updateDA($da) {
    ...
    if ($trouve) {
        ...
        if($num -eq $null) {
                ...
                $spItem["Etat"] = 6		  
                $spItem["Statut"] = "6 - Commande saisie dans Baan"	      

                incrementation}

        if($dateLIV.toString().length -lt 6) {Write-Host "Pas de date de livraison pour la DA n° " $numDA  }
                  else {$spItem["DateLivraison"] = $dateLIV}
	
	    if($dateLIV_EST.toString().Length -lt 6) {Write-Host "Pas de date de livraison estimée pour la DA n° " $numDA }
				else {$spItem["DateLivraisonEstim"] = $dateLIV_EST}
        
		if($dateCOM.toString().Length -lt 6) {Write-Host "Pas de date de commande pour la DA n° " $numDA }
				else {$spItem["DateCommande"] = $dateCOM}
