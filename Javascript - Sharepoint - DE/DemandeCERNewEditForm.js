...
var seuil = 0; // seuil au dela duquel la validation passe de 1 niveau à 2 niveaux
...
function updateBooleanValue(result, Name) {

    if (result) {
        updateInput0(Name, result);
        var elt = getInputElement(Name);
        elt.checked = true;}
...
function customPreSaveItem() {
    ...
    // Vérification que le responsable a bien été renseigné
    var responsableService = getUserField("RespServiceAffectation");
    if ((!responsableService) || (responsableService.length == 0) || (responsableService == '[]')) {
        errMessage += "Responsable du service manquant. ";
    }
    ...
    if (total > seuil) {
        ...
        if ((!respDirectionService) || (respDirectionService.length == 0) || (respDirectionService == '[]')) {
    ...
    if (elt.checked) {
        ...
        if ((!FournMotif) || (FournMotif.length == 0)) {
            errMessage += "Nom du motif justifiant le recours au fournisseur manquant. ";
        }
    }
    ...
    // Vérifier que les champs de numéro de référence et libellé immobilisation ne sont pas vides lorsque rattachemment ou remplacement immo non vides
    var rattachement = getInputElement("RattachementImmoExist"); 
    var remplacement = getInputElement("RemplacementImmoExist");
    var libelleImmo = getInput("LibelleImmoRattachement");
    var numeroCER = getInput("Numero_x0020_CER_x0020_Reference");

    if (((rattachement.checked) || (remplacement.checked) ) && ((libelleImmo.length ==0) || (!libelleImmo))) {
	errMessage += "Le libellé de l'immobilisation concernée doit être saisi !!";}

    if (((rattachement.checked) || (remplacement.checked) ) && ((numeroCER.length ==0) || (!numeroCER))) {
	errMessage += "Le numéro de la CER concernée doit être saisi !!";}

    // Vérification de la taille du champ de libellé FA7 inférieur à 60 caractères
    //var fa7 = getInput("Libelle_x0020_FASSET");
    //if (fa7.length >60) {
    //	errMessage += "Le libellé FA7 doit être inférieur à 60 caractères !!";
    //}	
...
function copyMainSelectedDAonQuerySucceeded(sender, args) {
    ...
    if (listItemEnumerator.moveNext()) {
        ...
        NomProjet = oListItem.get_item("Nom_x0020_Projet");
        LibelleFasset = oListItem.get_item("Libelle_x0020_FASSET");
        CodeBudget = oListItem.get_item("Code_x0020_budget");
        InclusBudget = oListItem.get_item("Inclus_x0020_au_x0020_budget");
        NumFicheFasset = oListItem.get_item("Numero_x0020_Fiche_x0020_Fasset");
        NumCERReferecne = oListItem.get_item("Numero_x0020_CER_x0020_Reference");
        AdresseLivraison = oListItem.get_item("Adresse_x0020_de_x0020_livraison");

        JustificationInvestissement = oListItem.get_item("JustificationInvestissement");
        DateLivraison = oListItem.get_item("DateLivraisonPrevue");
        EmplacementGeo = oListItem.get_item("EmplacementGeo");
        RattachementImmo = oListItem.get_item("RattachementImmoExist");
        RemplacementImmo = oListItem.get_item("RemplacementImmoExist");
        LibelleRattachement = oListItem.get_item("LibelleImmoRattachement");
        AccordDelegation = oListItem.get_item("AccordDelegationCER");

        CautionBancaire = oListItem.get_item("Caution_Bancaire");
        ConditionsPaiement = oListItem.get_item("Conditions_Paiement");
        ...
        updateInput("Nom_x0020_Projet", NomProjet);
        updateInput("Libelle_x0020_FASSET", LibelleFasset);
        updateInput("Code_x0020_budget", CodeBudget);

        updateInput("Numero_x0020_Fiche_x0020_Fasset", NumFicheFasset);
        updateInput("Numero_x0020_CER_x0020_Reference", NumCERReferecne);
        updateInput("Adresse_x0020_de_x0020_livraison", AdresseLivraison);
        updateInput("JustificationInvestissement", JustificationInvestissement);
        updateInput("DateLivraisonPrevue", DateLivraison);
        updateInput("EmplacementGeo", EmplacementGeo);
        updateInput("Conditions_Paiement", ConditionsPaiement);

        updateInput("LibelleImmoRattachement", LibelleRattachement);

        updateBooleanValue(AccordDelegation, "AccordDelegationCER");
        updateBooleanValue(RattachementImmo, "RattachementImmoExist");
        updateBooleanValue(RemplacementImmo, "RemplacementImmoExist");
        updateBooleanValue(InclusBudget, "Inclus_x0020_au_x0020_budget");        
        updateBooleanValue(CautionBancaire, "Caution_Bancaire"); 
    }
    this.status = "success";
}
